...
#############Mask preprocessing#############
# masks: border -> background, merges border into background
def border_to_background(mask):
    threes_mask = tf.equal(mask, 3)
    mask = tf.where(threes_mask, tf.cast(2, mask.dtype), mask)
    return mask
...
#background is set to 0 to have consistent filling in geometric augmentation with pictures
def swap_0_2(mask):
...
############Heatmap generation ---> Mask modification############
...
def second_perturbation(example, n=1):
    ...
    if len(image.shape) == 3:
        ...
        image = tf.squeeze(tf.squeeze(image, axis=0), axis=-1)
...
def third_perturbation(example, a=1.0):
    ...
    image = example["image"]
    image = tf.clip_by_value(image * a, 0.0, 1.0)
...
def fourth_perturbation(example, b=0.0):
    ...
    image = example["image"]
    image = tf.clip_by_value(image + b, 0.0, 1.0)
    return {
        "image": image,
        "segmentation_mask": example["segmentation_mask"]
    }
