...
from src.point_segmentation import random_loc, create_gaussian_heatmap, mask_heat_modification
...
def augment_image_mask_2d(image, mask, augment_color=None, augment_geometric=None, augment_noise_filter=None):
    batched_image = tf.expand_dims(image, axis=0)
    batched_mask = tf.expand_dims(mask, axis=0)
    batched_mask = tf.cast(batched_mask, tf.float32)
    ...
    image = tf.squeeze(batched_image, axis=0)
    mask = tf.squeeze(batched_mask, axis=0)
    mask = tf.cast(mask, tf.uint8)

    # get heatmap for random points of animal
    location = random_loc(mask_shape=mask.shape)
    heatmap = create_gaussian_heatmap(mask_shape=mask.shape, loc=location, sigma=20)
    mask = mask_heat_modification(mask, location)
    image_heatmap = tf.concat([image, heatmap], axis=-1)
    
    return image_heatmap, mask
...
def batch(data, augment='none', batch_size=16):
    ...
    if augment == 'training_2d':
        cached_train = data.cache()
        return cached_train.map(
            lambda image, mask: augment_image_mask_2d(image, mask, augment_color=True, augment_geometric=True, augment_noise_filter=False), 
            num_parallel_calls=tf.data.AUTOTUNE
        ).batch(batch_size).prefetch(tf.data.AUTOTUNE)
